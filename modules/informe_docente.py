def generarInformeDocente(num_preguntas, data, estudiantes):
    ...
    for idx, row in data_output.iterrows():
        ...
        if estudiante.shape[0] > 0:
            ...
            data_output.at[idx, "numeroID"] = (
                str(row["codigo"])[2:] + "-" + str(row["programa"])
            )

    data_output[["nombre", "codigo", "correctas", "nota", "numeroID"]].sort_values(
        "nombre"
    ).to_csv("output/listaCalificaciones.csv")
