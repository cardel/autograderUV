...
def generarInformeGrupal(
    ...
    for cod, tipo_examen in zip(data["codigo"].values, data["examen"].values):
        ...
        cnt_preg = 0
        for marcada_examen, correcta_examen in zip(
            respuestas_marcadas, respuestas_correctas
        ):
        ...
        examen_general_estudiante[codificacion_preguntas[int(tipo_examen)]] = (
            examen_por_estudiante
        )
    ...
    pdf.text(
        125,
        92,
        "Nota del 66.3% del grupo: "
        + "["
        + str(round(final_mean - final_std, 2))
        + " , "
        + str(round(final_mean + final_std, 2))
        + "]",
    )
    ...
    for label, value, dstd in zip(
        res_aprendizaje, promedio_grupal_aprendizaje, promedio_grupal_desviacion
    ):
        pdf.set_font("Times", "B", 16)
    ...
    pdf.text(
        10,
        35 + y,
        "Los temas que tienen menor promedio y mayor desviación estándar son los de mayor dificultad",
    )
    y += movy
    ...
    pdf.text(
        10,
        35 + y,
        "Una deviación estándar elevada indica que el manejo del tema no es uniforme dentro del grupo",
    )
    y += movy
    pdf.text(
        10,
        35 + y,
        "Se debe hacer mayor enfasis en ellos en el éxamen opcional y en las sesiones de estudio",
    )
