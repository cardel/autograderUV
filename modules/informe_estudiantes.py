...
def generarInformeEstudiantes(
    ...
    for cod, tipo_examen in zip(data["codigo"].values, data["examen"].values):
        ...
        pdf.text(
            5, 24, "Estudiante: " + data[data["codigo"] == cod]["nombre"].values[0]
        )
        pdf.text(
            5, 34, "Código: " + str(data[data["codigo"] == cod]["codigo"].values[0])
        )
        ...
        for correcta_examen, marcada_examen in zip(respuestas_correctas, marcadas):
            ...
            if not ("ANULADA" in correcta_examen):
                ...
                if numero_correctas >= 0.5:
                    ...
                    pdf.text(
                        160 + 15, 74 + pos, str(round(numero_correctas * 100, 2)) + "%"
                    )
                else:
        ...
        pdf.text(
            10,
            80 + pos,
            "Número de correctas: " + str(round(num_correctas_marcadas, 2)),
        )
        ...
        for label, value, preg in zip(
            res_aprendizaje, val_res_aprendizaje, preg_res_aprendizaje[int(tipo_examen)]
        ):
            ...
            pdf.text(
                10 + x,
                110 + pos + y,
                "Preguntas asociadas: " + str(np.array(index) + 1),
            )
            y += movy
        pos += y - 10
        pdf.set_font("Times", "B", 22)
